...
cxₑ, cyₑ, czₑ = ntuple(_ -> SphericalHarmonicExpansions.SphericalHarmonicCoefficients(L, R, true), 3)
cx, cy, cz    = ntuple(_ -> SphericalHarmonicExpansions.SphericalHarmonicCoefficients(L, R, true), 3)
...
Bₑ = [sphericalHarmonicsExpansion(c, x, y, z) for c in (cxₑ, cyₑ, czₑ)]
...
record(fig, "plots/$figname/animation-$(DEBUG ? 'd' : 'r').mp4", 1:FRAMES) do i
