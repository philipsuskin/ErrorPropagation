...
σ₁² = 2^2
σ₂² = ((asin(0.0115) / sqrt(3))^2, (1.1e-3 / sqrt(3))^2)
σ₃² = (0.05, [3e-6, 24e-6])
...
fig = GLMakie.Figure(size=(2000, 450), resolution=(8000, 1800), fontsize=56)

axs = [Axis3(fig[1, i == 1 ? i : i + 1]; height=Relative(0.9), aspect=:data, elevation=0.3π, azimuth=-0.75π, xlabel="x / mm", ylabel="y / mm", zlabel="z / mm", xlabeloffset=200, ylabeloffset=200, zlabeloffset=200, title=L"x", titlefont=:bold) for i = 1:4]
...
# crange_error = slice_extrema((x, y, z) -> map(f -> f(x, y, z), fsₑ), scales[2])
...
crange_error = (minimum([crange_error_x..., crange_error_y..., crange_error_z...]),
                maximum([crange_error_x..., crange_error_y..., crange_error_z...]))

cranges = [crange_field, crange_error]
...
function plotSlices(ax, f, crange, scale, min_arrow_norm=0.002)
  ...
  function slice_surface!(plane, ax, f; arrow_scale=0.4)
    ...
    arrows!(ax, ps, ns, fxaa=true, linecolor=:white, arrowcolor=:white, quality=256,
      linewidth=R / 90, arrowsize=Vec3f(R / 35, R / 35, R / 35), align=:center, transparency=true, alpha=0.7)
  end
...
plotSlices(axs[3], fsₑ[2], cranges[2], scales[2], nothing)
plotSlices(axs[4], fsₑ[3], cranges[2], scales[2], nothing)

Colorbar(fig[1, 2], limits=cranges[1], label=L"$\left|\left|\mathbf{B}\right|\right|_2$ / $\mathrm{mT}$", height=Relative(0.8), width=50)
Colorbar(fig[1, 6], limits=cranges[2], label=L"$\left|\left|\mathbf{B}\right|\right|_2$ / $\mu\mathrm{T}$", height=Relative(0.8), width=50)
